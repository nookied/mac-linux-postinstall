...
- **Camera install errors hidden by silent redirects** (`install_facetimehd`): a follow-up to the previous fix surfaced four more failure modes that were being swallowed: (1) Secure Boot blocks unsigned DKMS modules at load time with no visible error — added `mokutil --sb-state` pre-check that aborts cleanly with instructions if SB is on. (2) `dkms autoinstall 2>/dev/null` made build failures look identical to success — now captures output, checks `dkms status` after, prints captured log if not installed, suggests `sudo dkms autoinstall -k $(uname -r)` retry. (3) Firmware re-extraction used a fragile `find` across three guessed directories — replaced with explicit loop over the two known paths (`/usr/sbin/facetimehd_firmware_extract` and the libexec equivalent). (4) `modprobe facetimehd 2>/dev/null` hid the actual load error (e.g. "no such module" when DKMS failed) — now captures stderr and reports it with `lsmod` verification on success. *(Cherry-picked from `claude/fix-camera-detection-ej0wS` branch which had no PR. Branch deletable after this lands.)*
- **Whiptail checklist descriptions overflowed dialog right edge** (`extras.sh`): whiptail aligns all checklist descriptions to the same column (padded to the longest tag — `gnome_tweaks` = 12 chars), leaving ~57 chars for description text in a 76-wide dialog. Two manifest entries exceeded that: "mbpfan (sensible fan curves, runs cooler than Apple SMC default)" was 63 chars; "Developer tools (git, neovim, tmux, gcc, clang, ripgrep, etc.)" was 62 chars. Shortened to "Sensible fan curves (cooler than Apple SMC default)" (51) and "Developer tools (git, neovim, tmux, gcc, clang, etc.)" (53). *(Was PR #6, which had a stale base — applied directly to avoid merging in a regression.)*
- **Shellcheck SC1007 in color-disable lines** (`docs/install.sh`, `lib/common.sh`): when stdout is not a TTY the helpers blanked color variables with `_C_RESET= _C_BOLD= …` (one statement, all empty). Shellcheck (SC1007) reads space-separated `var=` patterns as commands rather than empty assignments. Replaced with explicit `_C_RESET=''; _C_BOLD=''; …` to make intent unambiguous and clear the warning.
- **Dead `REBOOT_NEEDED` env var** (`lib/common.sh`): `mark_reboot` set `REBOOT_NEEDED=true` but nothing read it — the bootstrap checks `[ -s "$REBOOT_FILE" ]` instead, because env vars don't propagate back across the sudo re-exec boundary. The variable was a vestige from before the file-based switch. Removed the dead assignment, updated the function header comment to describe the actual file-based mechanism, and updated CLAUDE.md §5 conventions to reference `mark_reboot` instead of the obsolete `REBOOT_NEEDED=true` pattern.
- **Shellcheck couldn't follow `lib/*.sh` source directives** (`tests/qa.sh`): `shellcheck -x` failed to resolve the dynamic `. "$(dirname "${BASH_SOURCE[0]}")/common.sh"` source paths because shellcheck's static analyzer doesn't expand the runtime expression, even with the `# shellcheck source=common.sh` directive. Fixed by adding `--source-path=SCRIPTDIR` to qa.sh's shellcheck invocation, which tells shellcheck to look relative to each script's own directory.
- **`detect_distro` QA test failed on macOS dev hosts** (`tests/qa.sh`): the test assumed `/etc/os-release` exists. macOS doesn't ship it, so the test failed when running QA locally during development. Wrapped in `if [ -r /etc/os-release ]` so it skips cleanly with a "non-Linux dev host" reason on macOS, while still running real verification on Linux.

- **FaceTime HD: wrong COPR name + wrong package mechanism** (`install_facetimehd`): real-hardware test on MBA 2017 + Fedora 44 (kernel 6.19.14) revealed the function silently did nothing — no COPR enabled, no packages installed, no module, no firmware, no `/etc/modules-load.d` entry. Two compounding bugs:
  1. **Wrong COPR name**: script tried `mulderje/facetimehd-dkms`, which **does not exist**. The actual COPR is `mulderje/facetimehd-kmod` (verified via COPR API: https://copr.fedorainfracloud.org/api_3/project/list?ownername=mulderje). Project list shows three: `facetimehd-kmod`, `intel-mac-rpms`, `facetimehd-kmod-pr`. None named `…-dkms`.
