...
- **Whiptail checklist item descriptions overflowing right edge** (`targets/macbookair7_2-fedora44/extras.sh`): whiptail aligns all checklist descriptions to the same column (padded to the longest tag width, here "gnome_tweaks" = 12 chars), leaving ~57 chars for description within a 76-wide dialog. Two entries exceeded this: "mbpfan (sensible fan curves, runs cooler than Apple SMC default)" (63 chars) and "Developer tools (git, neovim, tmux, gcc, clang, ripgrep, etc.)" (62 chars). Shortened to "Sensible fan curves (cooler than Apple SMC default)" (51 chars) and "Developer tools (git, neovim, tmux, gcc, clang, etc.)" (53 chars).
